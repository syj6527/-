...
이거 잘렸는데요?!" 한타 끝나면 "GG"로 정리. 용어는 폭격하되 교전 전까진 흥분을 아껴두세요.`, labels: ["중계","LIVE","한타"] },
    ...
    community: [
        { key:'youtube', label:'유튜브', subs: [
            { style: `【유튜브 댓글 st】(가이드 톤) 영상 밑 댓글창. 좋아요 많은 베댓 + 대댓글 구조. "0:42 이 부분 ㅋㅋㅋ 미쳤다 (👍 1.2만)", "알고리즘이 나를 여기로 이끌었다", "이걸 왜 이제 봤지". 타임스탬프 인용, 좋아요 수, "📌 고정 댓글", 구독·알림 드립. 영상 콘텐츠를 시청자가 댓글로 품평하는 톤.`, labels: ["댓글","베댓","구독"] },
        ] },
        { key:'blog', label:'블로그', subs: [
            { style: `【블로그 리뷰 st】(가이드 톤) 네이버 블로그 후기체. "안녕하세요 여러분~ 오늘은요!" 친근한 인사, 사진 자리 표시("(사진)"), "솔직 후기", "내돈내산", 별점, "*이 글은 협찬이 아닙니다*" 고지. 사소한 일을 정성스러운 리뷰 포스팅으로. 물결·"ㅎㅎ"·이모티콘 잦음.`, labels: ["블로그","후기","내돈내산"] },
        ] },
        { key:'community', label:'커뮤니티', subs: [
            { style: `【트위터(X) st】 인용RT·답멘·밈. 감정 폭발 단문("아 시발 존나 좋아서 정신잃음"), 자기인용 패러디, 인용 싸움 관전("인용에 A vs B 이뤄지고 있음"), "박제", "이 트윗 영원히 저주함", 조회 52만·RT·마음에 들어요 수치. 짧고 빠른 트윗체. 욕은 텐션용으로 자연스럽게.`, labels: ["실시간","타래","박제"] },
            { style: `【인스타 st】 감성 캡션 + 해시태그(#일상 #데일리 #오늘의기록), 위치 태그(📍), 댓글 반응("여기 어디예요?", "분위기 좋다ㅎㅎ"). 인플루언서·감성 톤. 셋 중 가장 순하고 예쁜 척하는 결 — 여기선 욕보다 오글거림이 무기.`, labels: ["피드","#태그","게시물"] },
            { style: `【팬커뮤·여초 st】 여초 팬 커뮤니티 게시판 댓글 도배체(특정 사이트 이름은 절대 언급하지 말 것). 종결어미 "~노/~음/~함/~네/~ㅇㅇ", "걍", "뇌절", "ㅂㅅ", ㅋㅋㅋ·ㅠㅠㅠ 도배. 번호 댓글이 우르르 달림. 욕하다 "그래도 사랑해❤", 비꼬다 "따학 감동이네 ㅠㅠ"로 손바닥 뒤집기. 최애 영업·흐뭇·"이 구역 떡밥 미쳤음".`, labels: ["최애","떡밥","팬심"] },
            { style: `【레딧 st】(가이드 톤) 영문 커뮤니티 번역체 느낌. "Edit: 금메달 감사합니다 친절한 stranger님", "TL;DR", 업보트/다운보트, "이거 r/○○에서 봤는데", 진지한 분석 답글 아래 "This." 한 줄 동의. 스레드 트리·"이 답글은 삭제되었습니다".`, labels: ["Reddit","업보트","TL;DR"] },
            ...
            { style: `【스레드 st】(가이드 톤) 익명 연속 스레드. 앵커 인용(">>12 이거 맞음"), "1빠", "그래서 결론이 뭐임", 짧은 익명 단답이 우르르 빠르게 핑퐁. 건조하고 빠른 익명 게시판 호흡.`, labels: ["스레드",">>앵커","익명"] },
        ] },
    ],
    scp: [
    ...
    radio: [
        { key:'story', label:'사연', subs: [
            { style: `【사연 코너 st】 라디오 DJ가 청취자 사연을 읽어주는 코너. RP 장면을 익명 사연("○○동 익명님", "라디오네임 ○○님이 보내주셨네요")으로 각색해 낭독하고, 다정하거나 능청맞은 사담을 붙인다. "오늘의 사연은요~", "어머 이분 좀 보세요", "사연자분, 제 말 듣고 계시죠?". 새벽 감성(나긋·위로)부터 정오 수다(밝고 텐션)까지 분위기를 살린다.`, labels: ["오늘의 사연","사연 배달","라디오네임"] },
        ] },
...
- interview(불경): "묻되 '어찌 손을 거두지 아니하느냐?' / 답하시되 '무거운 까닭이니라.' / 곁에서 린이 이르되 '거짓이로다.'"`,
        ...
        community: `당신은 인터넷 커뮤니티·SNS 유저들입니다. 진지한 RP 장면을 가볍고 시끄러운 인터넷 반응으로 받아쳐 웃깁니다. **세부 결**(유튜브/블로그/커뮤니티 등)에 맞춰 변주하세요. 트위터 톤으로 쏠리지 말고 지정된 결만 따르세요 — 예: 유튜브 댓글에 트위터 인용RT를, 블로그 후기에 커뮤 도배체를 섞지 말 것. 실제 커뮤·트위터처럼 **날것의 리듬**을 살리는 게 핵심 — 깔끔한 문어체로 쓰면 망합니다.
